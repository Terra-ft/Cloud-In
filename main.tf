...
resource "local_file" "private_key_pem" {
  content  = tls_private_key.mykey2.private_key_pem
  filename = "Mypro.pem"
}

# Save the public key to a local file
resource "local_file" "public_key_pem2" {
  content  = tls_private_key.mykey2.public_key_openssh
  filename = "publickey.ppk"
}
resource "aws_key_pair" "public_key_pem" {
...
resource "aws_instance" "njinx_server" {
  ...
  key_name                    = aws_key_pair.public_key_pem.key_name
